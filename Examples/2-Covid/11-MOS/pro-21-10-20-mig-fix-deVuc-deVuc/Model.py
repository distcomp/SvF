...
def createGr ( Task, Penal ) :
    ...
    Gr.conEQ0 = Constraint(testS.FlNodSm,rule=EQ0 )
    ...
    def fnT(t) : return nT__f.F([t])
 											# dVac(t)
    dVac = Funs[6];  dVac__f = dVac
    dVac__i = Funs[6].grd
    def fdVac(t) : return dVac__f.F([t])
 											# Mig(t)
    ...
    Gr.conEQ1 = Constraint(t.FlNodS,rule=EQ1 )
 											# N<1000000*0.30
    def EQ2 (Gr,ti) :
        return (
          fN(ti)<=1000000*0.30
        )
    Gr.conEQ2 = Constraint(t.FlNodS,rule=EQ2 )
 											# n0>= 0;
    n0 = Funs[13];  n0__f = n0
    ...
    np.InitByData()
    fnp = np__i
 											# R0(t)>=0;<=Imax;R0(tMax)=R0(tMax-1);d/dLoc2(R0(Loc2))<0;d/duL2(R0(uL2))>0;d/dLoc3(R0(Loc3))<0;d/duL3(R0(uL3))>0;d/dLoc4(R0(Loc4))<0;d/duL4(R0(uL4))>0;
    R0 = Funs[15];  R0__f = R0
    ...
    Gr.conEQ3 = Constraint(rule=EQ3 )
 											# d/dLoc2(R0(Loc2))<0
    def EQ4 (Gr,i__Loc2) :
        return (
          ((fR0((i__Loc2+Loc2.step))-fR0(i__Loc2))/Loc2.step)<=0
        )
    Gr.conEQ4 = Constraint(Loc2.FlNodSm,rule=EQ4 )
 											# d/duL2(R0(uL2))>0
    def EQ5 (Gr,i__uL2) :
        return (
          ((fR0((i__uL2+uL2.step))-fR0(i__uL2))/uL2.step)>=0
        )
    Gr.conEQ5 = Constraint(uL2.FlNodSm,rule=EQ5 )
 											# d/dLoc3(R0(Loc3))<0
    def EQ6 (Gr,i__Loc3) :
        return (
          ((fR0((i__Loc3+Loc3.step))-fR0(i__Loc3))/Loc3.step)<=0
        )
    Gr.conEQ6 = Constraint(Loc3.FlNodSm,rule=EQ6 )
 											# d/duL3(R0(uL3))>0
    def EQ7 (Gr,i__uL3) :
        return (
          ((fR0((i__uL3+uL3.step))-fR0(i__uL3))/uL3.step)>=0
        )
    Gr.conEQ7 = Constraint(uL3.FlNodSm,rule=EQ7 )
 											# d/dLoc4(R0(Loc4))<0
    def EQ8 (Gr,i__Loc4) :
        return (
          ((fR0((i__Loc4+Loc4.step))-fR0(i__Loc4))/Loc4.step)<=0
        )
    Gr.conEQ8 = Constraint(Loc4.FlNodSm,rule=EQ8 )
 											# d/duL4(R0(uL4))>0
    def EQ9 (Gr,i__uL4) :
        return (
          ((fR0((i__uL4+uL4.step))-fR0(i__uL4))/uL4.step)>=0
        )
    Gr.conEQ9 = Constraint(uL4.FlNodSm,rule=EQ9 )
 											# IS(t)>=0;
    IS = Funs[16];  IS__f = IS
    ...
    def fImun(t365) : return Imun__f.F([t365])
 											# Vacc(t365)>0.0;d/dt365(Vacc)<=0;Vacc(0)=effVacc;Vacc(max_Imun)=0.
    Vacc = Funs[21];  Vacc__f = Vacc
    Vacc__i = Var ( Funs[21].A[0].NodS,domain=Reals, bounds=(0.0,None), initialize = 1 )
    Vacc.var = Vacc__i ; Gr.Vacc =  Vacc__i
    Vacc.InitByData()
    def fVacc(t365) : return Vacc__f.F([t365])
 											# d/dt365(Vacc)<=0
    def EQ10 (Gr,i__t365) :
        return (
          ((fVacc((i__t365+t365.step))-fVacc(i__t365))/t365.step)<=0
        )
    Gr.conEQ10 = Constraint(t365.FlNodSm,rule=EQ10 )
 											# Vacc(0)=effVacc
    def EQ11 (Gr) :
        return (
          fVacc(0)==effVacc
        )
    Gr.conEQ11 = Constraint(rule=EQ11 )
 											# Vacc(max_Imun)=0.
    def EQ12 (Gr) :
        return (
          fVacc(max_Imun)==0.
        )
    Gr.conEQ12 = Constraint(rule=EQ12 )
    ...
    Gr.conEQ13 = Constraint(ta1.FlNodS,t.FlNodS,rule=EQ13 )
