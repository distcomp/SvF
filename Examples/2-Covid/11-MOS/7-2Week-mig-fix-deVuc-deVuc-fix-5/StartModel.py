...
Loc5=Grid('Loc5',to_npp('2021-09-29'),to_npp('2021-11-06'),1.0,'i__Loc5','Loc5')
Task.AddGrid(Loc5)
 											#       uL5    = [to_npp('2021-11-07'), to_npp('2021-11-08'), 1]
uL5=Grid('uL5',to_npp('2021-11-07'),to_npp('2021-11-08'),1.0,'i__uL5','uL5')
Task.AddGrid(uL5)
 											# maxFb = .8 #0.6  #9  #6
maxFb=.8
Task.AddDef('maxFb',[.8])
...
def fb(ta) : return b.F([ta])
 											# Set:    testS = [ 0, 10, 0.25 ]
testS=Grid('testS',0.0,10.0,0.25,'i__testS','testS')
Task.AddGrid(testS)
...
for tt in range(int(tMax),timeVacc-1,-1): Vac.grd[tt]=Vac.grd[tt-timeVacc]
 											...
 											# #Draw Exch
 											# Param: dayVac (t)
dayVac = Fun('dayVac',[t],True); 
Task.InitializeAddFun ( dayVac )
dayVac__f = dayVac
def fdayVac(t) : return dayVac.F([t])
...
fnp = np.grd
 											...
 											# Var:    R0  ( t ) >= 0; <= Imax;                d/dLoc2(R0(Loc2)) <0;  d/duL2(R0(uL2)) >0; \
 											# Var:    R0  ( t ) >= 0; <= Imax;                d/dLoc2(R0(Loc2)) <0;  d/duL2(R0(uL2)) >0;               d/dLoc3(R0(Loc3)) <0;  d/duL3(R0(uL3)) >0; \
 											# Var:    R0  ( t ) >= 0; <= Imax;                d/dLoc2(R0(Loc2)) <0;  d/duL2(R0(uL2)) >0;               d/dLoc3(R0(Loc3)) <0;  d/duL3(R0(uL3)) >0;               d/dLoc4(R0(Loc4)) <0;  d/duL4(R0(uL4)) >0; \ ##.005;  ###########################
 											# Var:    R0  ( t ) >= 0; <= Imax;                d/dLoc2(R0(Loc2)) <0;  d/duL2(R0(uL2)) >0;               d/dLoc3(R0(Loc3)) <0;  d/duL3(R0(uL3)) >0;               d/dLoc4(R0(Loc4)) <0;  d/duL4(R0(uL4)) >0;               d/dLoc5(R0(Loc5)) <0;  d/duL5(R0(uL5)) >0;
R0 = Fun('R0',[t],False); 
...
Vacc = Fun('Vacc',[t365],True); 
Task.InitializeAddFun ( Vacc, 'Vacc(t365).sol' )
Vacc__f = Vacc
...
Dat=Select ( 'Select nC As nCr,nT,nCR,t,t AS tr from DataAll As Dat' )
 											# Var: nCr(tr)
...
Dat=Select ( 'Select date As month,t AS tm from DataAll As Dat where month%100==1' )
 											# for i in t.NodS :
for i in t.NodS:
 											#     if i >= Dat.tm[-1]+30 :  Dat.AppendRec (0, Dat.tm[-1]+367./12)  #30)
    if i>= Dat.dat('tm')[-1]+30:Dat.AppendRec(0,Dat.dat('tm')[-1]+367./12)
 											# print ('MMM', Dat.tm)
print('MMM',Dat.dat('tm'))
 											# Var: month(tm)
...
DatTod=Select ( 'Select tm,ROWNUM as today from Dat As DatTod where today<= 2' )
 											# Var: today(tm)
