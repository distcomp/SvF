...
def createGr ( Task, Penal ) :
    ...
    T.var = Var ( T.A[0].NodS,T.A[1].NodS,domain=Reals, bounds=(0,None), initialize = 1 )
    Gr.T =  T.var
    T.InitByData()
    def fT(x,t) : return T.F([x,t])

    φ.var = Var ( φ.A[0].NodS,domain=Reals, bounds=(0,None), initialize = 1 )
    Gr.φ =  φ.var
    φ.InitByData()
    def fφ(x) : return φ.F([x])

    l.var = Var ( l.A[0].NodS,domain=Reals, bounds=(0,None), initialize = 1 )
    Gr.l =  l.var
    l.InitByData()
    def fl(t) : return l.F([t])

    r.var = Var ( r.A[0].NodS,domain=Reals, bounds=(0,None), initialize = 1 )
    Gr.r =  r.var
    r.InitByData()
    def fr(t) : return r.F([t])

    K.var = Var ( range (8), initialize = 0 )
    Gr.K =  K.var
    def fK(sT) : return K.F([sT])
    K.var[0].value = 1
 											# K(sT)>=0
