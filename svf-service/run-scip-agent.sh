#!/bin/bash
# Bootstrap script for Everest agent to run SCIP jobs inside SvF+solvers image.
# Usage in Everest plan (conceptual):
#   command /usr/local/bin/run-scip.sh ${docker_image_tag} ${nlname} ${options}
...
echo "Using SvF+solvers image for SCIP: ${IMAGE}"
...
docker run --rm \
  ...
  -w "${WORKDIR}" \
  "${IMAGE}" \
  /usr/local/bin/run-scip.sh "${NLNAME}" "${OPTS_FILE}" "$@" &
