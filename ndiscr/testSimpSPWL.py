...
for j in m.J:
    m.Fx[j] = (1/2)*(m.F[0,j] + A(1, j)*(m.x - m.X[0]) + m.F[0,j] + A(Nx,j)*(m.x - m.X[Nx])) + \
              (1/2)*pyo.quicksum(
                  (m.F[i,j] - 2*m.F[i-1,j] + m.F[i-2,j]) * pyo.sqrt((m.x - m.X[i - 1]) ** 2 + m.eps ** 2) for i in pyo.RangeSet(2, Nx))
...
Ax = lambda j: m.Fx[j] - m.Fx[j-1]
m.Fxy = (1/2)*(m.Fx[0] + Ax(1)*(m.y - m.Y[0]) + m.Fx[Ny] + Ax(Ny)*(m.y - m.Y[Ny])) + \
        (1/2)*pyo.quicksum((m.Fx[j] - 2*m.Fx[j-1] + m.Fx[j-2])*pyo.sqrt( (m.y - m.Y[j-1])**2 + m.eps**2) for j in pyo.RangeSet(2, Ny))
# (1/2)*pyo.quicksum((m.Ax[j] - m.Ax[j - 1]) * pyo.sqrt((m.y - m.Y[j - 1]) ** 2 + m.eps ** 2) for j in pyo.RangeSet(2, Ny))
