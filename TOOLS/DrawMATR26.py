...
for n_arg in range ( len (sys.argv)-1 ) :
  ...
  fun = FunFromFile ( file_name )
  fun.myprint()

  if fun.dim == 1 :    ################################### MATR
            cols = [ fun.A[0].name, fun.V.name ]

            for g in range (1,2) : # tbl.shape[1]) :

                #if color == 1 : color = 2; continue
                color = n_arg
                while color >= len (colrs) : color -= len (colrs)
                if Transp :
                    tbl_x = fun.grd  # [:,g]
                    tbl_y = fun.A[0].Val
                else      :
                    tbl_y = fun.grd  # [:,g]
                    tbl_x = fun.A[0].Val
                ax.plot ( tbl_x, tbl_y, colrs[color], label=cols[g],  linestyle='-', linewidth=line_w, \
                              markersize=0.5, marker='o', markerfacecolor='#FFFFFF' )
#                if color ==0 :
 #                   ax.plot ( tbl_x, tbl_y, colrs[color], label=cols[g],  linestyle='-', linewidth=line_w, \
  #                            markersize=0.5, marker='o', markerfacecolor='#FFFFFF' )
   #             elif color == 1 :
    #                ax.plot ( tbl_x, tbl_y, colrs[color], label=cols[g], linestyle=':', linewidth=, markersize=1, marker='o' )
     #           elif color == 2 :
      #              ax.plot ( tbl_x, tbl_y, colrs[color], label=cols[g], linestyle='-', linewidth=0, markersize=0.25, marker='o' )
          #      else :    
           #         ax.plot ( tbl_x, tbl_y, colrs[color], label=cols[g], markersize=2, marker='o', linewidth=0 )
                color = color + 1
                if color >= len (colrs) : color = 0
