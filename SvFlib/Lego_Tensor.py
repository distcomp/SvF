...
class Tensor (Object) :
    ...
    #    self.dim = len(Sizes)
        self.Allocate_tensor(tuple(Sizes))
 ...
 #       self.grd = np.ones(tuple(Sizes), np.float64)
        if self.dim == 0: self.grd = np.full((1), fill_value=self.Finitialize, dtype=np.float64 )
        else :            self.grd = np.full(self.Sizes, fill_value=self.Finitialize, dtype=np.float64)
