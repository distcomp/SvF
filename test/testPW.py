...
def addOde1x(model: pyo.ConcreteModel, setT: list, yMin: float, yMax: float, Ny: int, eps: float, useEta = True):
    Nx = len(setT)-1
...
def initXtFy(model, setT, yLo, yUp, Ny):
    Nx = len(setT) - 1
    model.setTidx = pyo.RangeSet(0, Nx)
    ...
    # Create y-meshgrid
    model.setYidx = pyo.RangeSet(0, Ny)
    model.dy = pyo.Param(initialize=(yUp - yLo) / Ny)
    dy = (yUp - yLo) / Ny
    ...
    def printData():
        ...
        print("Nx = ", Nx, " Ny = ", Ny)
        print("meshT: ", [t for t in setT])
        print("meshY: ", [y for y in theModel.meshY])
        print("||||||||||||||||||||||||||||||||")
...
if __name__ == "__main__":
    ...
    #
    # Nx = len(setT)-1
    # theModel.setTidx = pyo.RangeSet(0,Nx)
    #
    # def meshT_init(m, k):
    #     return setT[k]
    # theModel.meshT = pyo.Param(theModel.setTidx, initialize=meshT_init)
    #
    # # Create y-meshgrid
    # theModel.setYidx = pyo.RangeSet(0,Ny)
    # theModel.dy = pyo.Param(initialize=(yUp - yLo)/Ny)
    # dy = (yUp - yLo)/Ny
    #
    # def meshY_init(m, i):
    #     return yLo + i*dy
    # theModel.meshY = pyo.Param(theModel.setYidx, initialize=meshY_init)
    #
    # def printData():
    #     print("||||||||||| DATA |||||||||||||||")
    #     print("Nx = ", Nx, " Ny = ", Ny)
    ...
    # printData()
    #
    # theModel.Fy = pyo.Var(theModel.setYidx, within=pyo.Reals)
    # theModel.Xt = pyo.Var(theModel.setTidx, within=pyo.Reals)
# addOde1(model: pyo.ConcreteModel, meshT, varXt, Nx: int, meshY, varFy, yMin: float, yMax: float, Ny: int, eps: float, useEta = True)
    initXtFy(theModel, setT, yLo, yUp, Ny)
    addOde1(theModel, setT, theModel.Xt, Nx, theModel.meshY, theModel.Fy, yLo, yUp, Ny, float(0.1))
    ...
    initXtFy(theModel, setT, yLo, yUp, Ny)
    addOde1(theModel, setT, theModel.Xt, Nx, theModel.meshY, theModel.Fy, yLo, yUp, Ny, float(0.1), useEta=False)
    theModel.pprint()
