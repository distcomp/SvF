...
model.idx_set = Set(initialize=[1,2])
...
model.SOS_indices = Set(model.idx_set,dimen=2, ordered=True, initialize=SOS_indices_init)
...
model.x = Var(model.idx_set) # domain variable
model.Fx = Var(model.idx_set) # range variable
model.y = Var(model.sos_var_indices,within=pyo.NonNegativeReals) # SOS2 variable
...
model.constraint1 = Constraint(model.idx_set,rule=constraint1_rule)
model.constraint2 = Constraint(model.idx_set,rule=constraint2_rule)
model.constraint3 = Constraint(model.idx_set,rule=constraint3_rule)
model.SOS_set_constraint = SOSConstraint(model.idx_set, var=model.y, index=model.SOS_indices, sos=2)
...
write_nl_only(model, "test_sos2_piecewise.nl", symbolic_solver_labels=True)