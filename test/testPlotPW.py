...
def plotScaledModelPW(model: pyo.ConcreteModel, XTscaler: XTScaling, txDataValues, savefigFileName):
    ...
    Xt = np.array([XTscaler.sx2x(pyo.value(model.Xt[k])) for k in tIndex], dtype=float)
    ...
    y = np.array([XTscaler.sx2x(j)  for j in xIndex], dtype=float)
    Fx = np.array([pyo.value(model.Fx[j]) for j in xIndex], dtype=float)

    fig, ax = plt.subplots()
    ax.plot(y, Fx, label='F(x)')
